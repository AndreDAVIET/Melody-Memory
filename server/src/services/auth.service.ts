...
export class AuthService {
    ...
    /**
     * Sign a user relative to the pseudo and password in parameter.
     * Compare the password with the user relative to the pseudo account from the database.
     * Create a new token bearer which contain the pseudo and user id.
     * Return the token, pseudo annd user id.
     * 
     * @param pseudo user pseudo
     * @param password user password
     */
    async signin(pseudo: string, password: string) {
        const user = await this.repository.findByPseudo(pseudo);
        if (!user) {
    ...
    async verifyToken(req: Request, res: Response, next: Function) {
        ...
        if (!bearerToken) {
            ...
            try {
                ...
                const user = await UsersRepository.getInstance().findByPseudo(results.pseudo);
                req.user = {
                    ...user,
                    password: undefined
                };
